...
function ResultsPage({ setRecentWords, recentWords }) {
  ...
  const fetchDataForWord = async () => {
    try {
      ...
      const res = await fetch(
        `https://api.dictionaryapi.dev/api/v2/entries/en/${wordToSearchFor}`
      );
  ...
  const mappedPartOfSpeech =
    dataFromFetch &&
    dataFromFetch.meanings.map((meaning, index) => {
      return (
        <PartOfSpeech
  ...
  const mappedDefinitions =
    dataFromFetch &&
    dataFromFetch.meanings[partOfSpeechIndex].definitions.map(
      (definition, index) => {
        if (index <= 3) {
          return (
            <Definition
  ...
  return (
    <>
          ...
          <div className="result-from-search">
            <header>
              <div className="text-and-icon">
                ...
                <button onClick={() => playAudio(audioRef)}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
                    height="24"
            ...
            <div className="definitions">{mappedDefinitions}</div>

            <p className="external-link">
              Read More:{" "}
              <a href={dataFromFetch?.sourceUrls[0]} target="_blank">
                {dataFromFetch?.sourceUrls[0]}
              </a>
            </p>
          </div>
        ) : (
      ...
      <audio ref={audioRef} src={dataFromFetch?.phonetics[0]?.audio}></audio>
